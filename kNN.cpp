...
int KNeighborsClassifier::PredictSingleCoordinate(const std::vector<double>& vectorToPredict) const {
    ...
    int i = 0;
    for ( ; i < KNeighbors; ++i) {
        queueDistanceLabelPair.push(std::make_pair(MeasureDistanceCoordinates(vectorToPredict, coordinateMatrix[i]), i));
    }
    ...
    for ( ; i < coordinateMatrix.size(); ++i) {
        ...
        if (distanceOfIthString < queueDistanceLabelPair.top().first) {
            ...
            queueDistanceLabelPair.push(std::make_pair(distanceOfIthString, i));
        }
    }
